...
export const StorageManager: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const calculateStorage = () => {
    ...
    // Use Object.keys() instead of for...in to avoid iterating inherited properties
  ...
  const clearOldDiagrams = () => {
    if (window.confirm('This will remove all saved diagrams. Are you sure?')) {
      // Use Object.keys() instead of for...in to avoid iterating inherited properties
      const keysToRemove = Object.keys(localStorage).filter(key => key.startsWith('stackdraw-'));
  ...
  const exportAllDiagrams = () => {
    const diagrams = localStorage.getItem('stackdraw-diagrams');
    if (diagrams) {
      const blob = new Blob([diagrams], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `stackdraw-backup-${Date.now()}.json`;
      a.click();
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <ul style={{ fontSize: '14px' }}>
            <li>StackDraw diagrams: {formatBytes(storageInfo.diagrams)}</li>
