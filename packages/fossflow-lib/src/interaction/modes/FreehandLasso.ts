...
const getItemsInFreehandBounds = (
  ...
  // Check all connectors - select if both anchor points are within the polygon
  scene.connectors.forEach((connector: any) => {
    ...
    // Connector is selected if both anchors are within the polygon
    const allAnchorsInside = anchorTiles.every((tile: any) =>
      tile && isPointInPolygon(tile, pathTiles)
    );
...
export const FreehandLasso: ModeActions = {
  ...
  mouseup: ({ uiState, scene }) => {
    ...
    if (uiState.mode.path.length >= 3 && !uiState.mode.selection) {
      ...
      // Sync selection with selectedItems
      if (items.length > 0) {
        uiState.actions.setSelectedItems(items);
      }

      // Keep selection for drag functionality, but clear the drawing path
      uiState.actions.setMode(
        produce(uiState.mode, (draft) => {
          if (draft.type === 'FREEHAND_LASSO') {
            draft.selection = {
