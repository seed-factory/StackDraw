...
export const Connector: ModeActions = {
  ...
  exit: ({ scene }) => {
    ...
    // End any pending transaction if user exits mode mid-operation
  ...
  mousedown: ({ uiState, scene, isRendererInteraction }) => {
    ...
    if (uiState.connectorInteractionMode === 'click') {
      ...
      if (!uiState.mode.startAnchor) {
        ...
        // Begin transaction before creating - saves history once
        ...
        if (currentMode.id) {
          ...
          // If connector doesn't exist, end transaction, reset mode and return
          if (!connector) {
            scene.endTransaction();
            uiState.actions.setMode({
              type: 'CONNECTOR',
          ...
          // End transaction when second click completes the connection
          scene.endTransaction();
  ...
  mouseup: ({ uiState, scene }) => {
    ...
    if (uiState.connectorInteractionMode === 'drag') {
      // End transaction when drag is complete
      scene.endTransaction();
