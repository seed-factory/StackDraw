...
export const useInteractionManager = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      } else if (hotkeyMapping.delete && key === hotkeyMapping.delete) {
        e.preventDefault();
        uiState.actions.setMode({
          type: 'DELETE',
          showCursor: true
  ...
  const processMouseUpdate = useCallback(
    (nextMouse: Mouse, e: SlimMouseEvent) => {
      ...
      // Always update mouse position, regardless of whether mode handles this event
      ...
      // Call mode-specific handler if it exists
      if (modeFunction) {
