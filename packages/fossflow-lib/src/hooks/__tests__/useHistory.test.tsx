...
// History state for mocks
const mockModelHistory = { past: [] as any[], future: [] as any[] };
const mockSceneHistory = { past: [] as any[], future: [] as any[] };
...
describe('useHistory', () => {
  beforeEach(() => {
    ...
    // Reset history state
    mockModelHistory.past = [];
    mockModelHistory.future = [];
    mockSceneHistory.past = [];
    mockSceneHistory.future = [];
  ...
  describe('undo/redo basic functionality', () => {
    ...
    it('should perform undo when model store has history', () => {
      mockModelHistory.past = [{ state: 'old' }];
    ...
    it('should perform undo when scene store has history', () => {
      mockSceneHistory.past = [{ state: 'old' }];
    ...
    it('should perform redo when model store has future', () => {
      mockModelHistory.future = [{ state: 'new' }];
  ...
  describe('history management', () => {
    ...
    it('should check both stores for redo capability', () => {
      ...
      // Only scene has redo
      mockModelHistory.future = [];
      mockSceneHistory.future = [{ state: 'new' }];
      mockModelStore.canRedo.mockReturnValue(false);
  ...
  describe('edge cases', () => {
    it('should handle missing store actions gracefully', () => {
      ...
      useModelStore.mockImplementation((selector: any) => {
        const state = { actions: undefined, history: undefined };
        return selector ? selector(state) : state;
      });
      useSceneStore.mockImplementation((selector: any) => {
        const state = { actions: undefined, history: undefined };
        return selector ? selector(state) : state;
      });
      ...
      useModelStore.mockImplementation((selector: any) => {
        const state = { actions: mockModelStore, history: mockModelHistory };
        return selector ? selector(state) : state;
      });
      useSceneStore.mockImplementation((selector: any) => {
        const state = { actions: mockSceneStore, history: mockSceneHistory };
        return selector ? selector(state) : state;
      });
    });
