...
export const useScene = () => {
  ...
  const layersList = useMemo(() => {
    return [...(currentView.layers ?? [])].sort((a, b) => a.order - b.order);
  }, [currentView.layers]);
  ...
  const updateGroup = useCallback(
    (id: string, updates: Partial<ItemGroup>) => {
      if (!currentViewId) return;
  ...
  const moveItemsToLayer = useCallback(
    (itemIds: Array<{ type: string; id: string }>, layerId: string | null) => {
      if (!currentViewId) return;

      saveToHistoryBeforeChange();
      const newState = reducers.view({
        action: 'MOVE_ITEMS_TO_LAYER',
        payload: { itemIds, layerId },
        ctx: { viewId: currentViewId, state: getState() }
      });
      setState(newState);
    },
  ...
  // Begin a long-running transaction (e.g., for drag operations)
  // This saves history once and prevents further saves until endTransaction is called
  ...
  // End a long-running transaction
  ...
  /**
   * Find all orphaned ModelItems (not referenced by any ViewItem)
   */
  const findOrphanedModelItems = useCallback(() => {
    return reducers.findOrphanedModelItems(getState());
  }, [getState]);

  /**
   * Clean up all orphaned ModelItems in one operation
   */
  const cleanupOrphanedModelItems = useCallback(() => {
    saveToHistoryBeforeChange();
    const newState = reducers.cleanupOrphanedModelItems(getState());
    setState(newState);
  }, [getState, setState, saveToHistoryBeforeChange]);
  ...
  return {
    ...
    isInTransaction,
    placeIcon,
    findOrphanedModelItems,
    cleanupOrphanedModelItems
  };
};
